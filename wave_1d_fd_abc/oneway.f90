...
contains
        ...
        subroutine fd_abc(f, fp, model_padded, dt, dx, i, direction,   &
                ...
                r = direction * model_padded(i) * dt / dx
                ...
                if (method == 1) then
                        ...
                        ! Lax-Friedrichs
                        one_way = 1 / 2.0 * (f(i + 1) + f(i - 1)) -            &
                                r / 2 * (f(i + 1) - f(i - 1))
                else if (method == 3) then
                        ! Leapfrog
                        one_way = fp(i) - r * (f(i + 1) - f(i - 1))
                else if (method == 4) then
                        ! Lax-Wendroff
                        one_way = fp(i) - r/2 * (f(i + 1) - f(i - 1)) +          &
                                (r**2)/2 * (f(i + 1) - 2 * f(i) + f(i - 1))
                else
